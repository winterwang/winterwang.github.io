...
- 通過比較，我們選擇了能夠讓觀察時間發生概率最高的參數取值 ($\pi=0.4$)。
...
1. $P(\underline{x}|\theta)$ 可以是概率（離散分佈）方程，也可以是概率密度（連續型變量）方程。對於此方程，$\theta$ 是給定的，然後再計算某些時間發生的概率。
...
要注意的是，微分不提定總是能幫助我們求額的似然方程的最大值。如果說參數本身的定義域是有界限的話，微分就行不通了：
...
### 極大似然估計 (maximum likelihood estimator) 的性質：
